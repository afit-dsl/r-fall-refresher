...
```{r setup, echo=FALSE}
knitr::opts_chunk$set(echo = TRUE, fig.align = 'center', fig.width = 7, message = F, comment = NA)
m = 5
b = 3
```
...
for(i in 1:length(slope)) {
    ...
    for(j in 1:length(intercept)) {
        ...
        loss_n[i,j] <- sum((y_p - slope[i] * x_p - intercept[j]))
...
optim(par = c(1,1), # provide starting values for m and b
      ...
      x = x_p,      # provide values for known parameters
      y = y_p,      # provide values for known parameters
      control = list(fnscale = 1))
```
...
for(i in 1:length(slope)) {
    ...
    for(j in 1:length(intercept)) {
        ...
        loss_a[i,j] <- sum(abs(y_p - slope[i] * x_p - intercept[j]))
...
for(i in 1:length(slope)) {
    ...
    for(j in 1:length(intercept)) {
        ...
        loss2[i,j] <- sum((y_p - slope[i] * x_p - intercept[j])^2)
